...
void Image::bilinearInterpScalation(float amount)
{
    ...
    // The lambda is restricted and can not have (this) pointer
    ...
    // Precalculating the inverse of the size ratio so inside the loop there will be a mul instead of a div
    ...
    parallel_for_each(dst_size, [=](index<2> idx) restrict(amp)
    {
        ...
        unsigned int gx_fx16 = direct3d::imin(idx[1] * inv_w_ratio_fx16 - half_fx16, max_w_fx16);
        unsigned int gy_fx16 = direct3d::imin(idx[0] * inv_h_ratio_fx16 - half_fx16, max_h_fx16);
        // Top-Left pixel from source image
        int gx = gx_fx16 >> 16; // FX24 not possible because this integer values grows past 255
        int gy = gy_fx16 >> 16;
        ...
        // Strange shifts exist due to the fixed point multiplication (fractional portion grows as a side effect)
...
void Image::allEffectsOptimized()
{
    ...
    parallel_for_each(img_size, [=](index<2> idx) restrict(amp)
    {
        ...
        // Grayscale
        val = r * 0.2627f + g * 0.6780f + b * 0.0593f;

        // +20% brightness and clamp to in case of char overflow
        val = direct3d::imin(val * 1.2f, 255);
